...
Вы можете переопределить эти значения через переменные окружения `CHATGPT_MODEL_PRIMARY` и `CHATGPT_MODEL_FALLBACK`. Если указать платную модель, убедитесь, что на балансе OpenRouter есть средства: при нулевом балансе сервис вернёт ошибку 402, а ответы не будут приходить.
