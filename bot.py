        ...
        'items': [
            {
                ...
                'image_url': 'https://i.imgur.com/3RNcZM5.jpeg',
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- НАЧАЛО НОВОГО БЛОКА ОБРАБОТКИ ПОДДЕРЖКИ ---
    if user_state == 'awaiting_support_message' and str(user.id) != ADMIN_CHAT_ID:
        # Немедленно сбрасываем состояние, чтобы следующие сообщения были обычными
        ...
        # Готовим для админа информацию и кнопку "Ответить"
        ...
        reply_button = [[
            InlineKeyboardButton(
                ...
                # Передаем ID пересланного сообщения, чтобы админ мог на него ответить
        ...
        if target_user_id:
            try:
                ...
                await context.bot.copy_message(
                    ...
                    message_id=update.message.message_id,
                    reply_to_message_id=message_id_to_reply # Отвечаем на конкретное сообщение
                )
    ...
    # ... (здесь идет остальной код функции handle_message без изменений) ...
    # ... (обработка broadcast, awaiting_id_submission, кнопок меню и т.д.) ...
...
async def user_actions_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "approve":
        ...
        # Очищаем ID для ответа, так как это общий режим "написать сообщение"
